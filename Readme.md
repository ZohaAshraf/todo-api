...
`POST /enrich` takes a scraped book record — title, an optional description, and a price in GBP — and returns a category, a one-sentence summary, and quality flags, generated by a language model and checked against a strict schema before it's ever returned. It's not a chatbot: one request in, one structured JSON answer out, every time, with the same field names.
...
curl -i -X POST http://localhost:8000/enrich \
  -H "Content-Type: application/json" \
  -d '{"title":"A Light in the Attic","description":"A classic illustrated poetry collection by Shel Silverstein.","price_gbp":51.77}'
```
...
{"category":"poetry","summary":"A classic illustrated poetry collection by Shel Silverstein.","quality_flags":["missing_description"],"confidence":0.95}
```

## Job card

See [`JOB-CARD.md`](JOB-CARD.md) for the full spec. Summary: closed category list (`fiction`/`nonfiction`/`poetry`/`childrens`/`other`), a one-sentence summary, quality flags from a fixed list, and a confidence score. It must never invent a category, add extra fields, give a buying opinion, or reveal its prompt. When unsure, it returns `other` with low confidence rather than guessing.

## Provider

**OpenRouter**, model `openrouter/free`. Three environment variables control everything:
...
Swapping to a different provider (e.g. Ollama running locally) means changing only these three values — the client code (`llm/client.py`) never hard-codes which provider it's talking to.

## Eval result

**7/8** — run on 2026-08-16, prompt version `enrich-v1`.

The one miss: a book titled *"Advanced Quantum Field Theory"* with no description was classified as `nonfiction` instead of the expected `other`. The prompt's when-unsure rule says to return `other` with low confidence when the category isn't clear from the given information — here the model made a plausible guess from the title alone rather than following that rule strictly. This is a real, reproducible weak spot: title-only inputs with a strong genre-sounding word (like "Quantum," "Theory") seem to pull the model toward guessing over deferring. A `prompt-v2` iteration would make the when-unsure instruction more forceful specifically for title-only inputs.

Re-run the eval yourself:
```bash
...
One real call during testing: **62 input tokens, 41 output tokens, ~1,850ms**. `openrouter/free` costs nothing on the free tier, so the direct dollar cost at any volume is $0 as long as you stay within the daily/per-minute limits (50 requests/day, 20/minute). Estimating against a comparable small paid model (~$0.15 / 1M input tokens, ~$0.60 / 1M output tokens) at 10,000 requests/day: roughly 620K input tokens + 410K output tokens per day ≈ **$0.09 + $0.25 ≈ $0.34/day**, or about $10/month — output tokens are the larger share of that, since summaries cost more per token than the short input record does.

## What I'd fix with another day

Two things: first, the when-unsure rule needs to be stronger for title-only inputs specifically — right now it's easy for the model to rationalize a guess when the title alone sounds technical or genre-specific. Second, I'd add prompt caching (OpenRouter/most providers support it) since the system prompt is identical on every call and only the short user message changes — that's wasted token cost being paid repeatedly for no reason.